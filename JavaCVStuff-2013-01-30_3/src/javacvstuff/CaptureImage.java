...
public class CaptureImage {
    ...
    private static void captureFrame(CanvasFrame canvas) {
        ...
        try {
            ...
            //CanvasFrame canvasCool = new CanvasFrame("Awesome");
            //canvasCool.setDefaultCloseOperation(javax.swing.JFrame.EXIT_ON_CLOSE);
            IplImage img;
           // imgcool = new IplImage()
            //IplImage hsv;
            //IplImage canny;
            IplImage dst;
...
//                                    + (240 - ((polygons.get(i).getVertex(3).y() + (polygons.get(i).getVertex(2).y())) / 2)));
                            ...
                            try {
                                if (distance < 150) {
                                    ...
                                    boolean xcheck = NetworkTable.getTable("rob").containsKey("x");
                                    System.out.print(xcheck);
                                    if (xcheck) {
                                        System.out.println(NetworkTable.getTable("rob").getNumber("x"));
                                    }
                                    else
                                    {
                                        System.out.println("---");
